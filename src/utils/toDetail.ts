...
export function useDetail() {
  ...
  function toDetail(
    ...
    // ⚠️ 这里要特别注意下，因为vue-router在解析路由参数的时候会自动转化成字符串类型，
    ...
    if (model === "query") {
      ...
      // 路由跳转
      router?.push({ name: _router.name, query: parameter });
    } else if (model === "params") {
      useMultiTagsStoreHook().handleTags("push", {
        path: `${_router.path}/:id`,
        name: _router.name,
        params: parameter,
        meta: {
          title: _router.meta.title
      ...
      router?.push({ name: _router.name, params: parameter });
    }
  }

  // 用于页面刷新，重新获取浏览器地址栏参数并保存到标签页
  const initToDetail = (model: "query" | "params") => {
    if (getParameter) toDetail(getParameter, model, route);
  };
